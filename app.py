...
st.markdown("""
<style>
body { background: linear-gradient(120deg,#1F4E79,#4A90E2);}
.main-container { background-color: rgba(255,255,255,0.95); padding:40px; border-radius:18px; box-shadow:0px 10px 30px rgba(0,0,0,0.2);}
.title { text-align:center; font-size:48px; font-weight:800; color:#1F4E79;}
.subtitle { text-align:center; font-size:18px; color:#555; margin-bottom:40px;}
.section-title{ font-size:26px; font-weight:700; margin-bottom:15px; color:#1F4E79;}
.prediction-card { background: linear-gradient(135deg,#43cea2,#185a9d); padding:30px; border-radius:15px; text-align:center; font-size:30px; font-weight:bold; color:white; margin-top:20px; box-shadow:0px 6px 20px rgba(0,0,0,0.2);}
.stButton>button { background: linear-gradient(135deg,#1F4E79,#4A90E2); color:white; font-size:20px; font-weight:600; border-radius:12px; height:55px; width:100%; border:none;}
.stButton>button:hover{ transform:scale(1.02); background: linear-gradient(135deg,#154360,#1F4E79);}
...
def load_model():
    try:
        # Google Drive link to your compressed model
...
st.markdown('<p class="subtitle">Predict resale value of a car using Machine Learning</p>', unsafe_allow_html=True)
...
st.write("""
This app predicts the resale price of a car using a Machine Learning model.
